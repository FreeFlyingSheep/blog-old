...
简单介绍 LoongArch 的汇编指令和一些 ABI 规范，相关文档见[文档仓库](https://github.com/loongson/LoongArch-Documentation)。

<!--more-->

## 寄存器

LoongArch 的寄存器较多，寄存器调用约定基本和 RISC-V 相同。
...
`$fcsr0`-`$fcsr3` 一共 4 个浮点控制状态寄存器（FCSR），每个长度都是 32 位，可以读写，具体功能可以参考手册。
...
不需要担心 `b` 和 `B` 冲突，因为指令后缀只有字节，没有比特。
...
我的理解是，当程序运行时，`PC` 肯定是位于代码段的，需要获取一些离当前 `PC` 固定偏移的数据段的数据地址时，就可以使用这些指令。
...
这是手册中提到的，属于指令别名，等价于 `andi $zero, $zero, 0`，但我也把它归在伪指令里了。
...
从加载途径划分，加载地址可以细分为 PC 相对位移加载（`la.pcrel`）、GOT 表加载（`la.got`）和绝对地址加载（`la.abs`）。
但不推荐使用这几个宏，除非你确定你非常清楚在做什么，通常的程序交给编译器去自己判断会更好。
